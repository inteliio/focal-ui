"use client";
import { BlogType } from "@/app-layout/ai-blog/page";
      ...
      <div className="container">
            ...
            <div key={blog.slug} className="underline-hover-effect group max-w-[370px]">
              <Link href={`/ai-blog/${blog.slug}`} className="block">
                <figure className="h-[388px] overflow-hidden xl:min-w-[360px]">
                  <Image width={360} height={388} src={blog.thumbnail} className="h-full w-full object-cover transition-all duration-500 group-hover:rotate-3 group-hover:scale-125" alt="Blog Images" />
                </figure>
              </Link>
              <p className="font-poppins mb-5 mt-[30px] text-sm font-normal uppercase leading-[1.1] tracking-[1.12px]">{blog.date}</p>
              <Link href={`/ai-blog/${blog.slug}`}>
                <div className="blog-title mb-9">
                  <h3 className="text[25px] md:text-3xl lg:text-4xl lg:leading-[1.2] lg:tracking-[-0.72px]">{blog.title}</h3>
                </div>
              </Link>

              <Link href={`/ai-blog/${blog.slug}`} className="rv-button rv-button-primary2 block w-full md:inline-block md:w-auto">
        ...
        <article className="mt-12 md:mt-[70px] [&>*:not(last-child)]:mb-10">
          {currentPageData?.slice(3)?.map((blog) => (
            <RevealWrapper key={blog.slug} className="underline-hover-effect group flex flex-col-reverse items-center justify-center gap-x-6 gap-y-10 border dark:border-dark md:justify-normal lg:flex-row lg:p-10">
              <div className="flex-1 max-lg:self-center max-lg:px-7 max-lg:pb-7">
                <p className="font-poppins text-sm font-normal uppercase leading-[1.1] tracking-[1.12px]">{blog.date}</p>
                <Link href={`/ai-blog/${blog.slug}`}>
              ...
              <Link href={`/ai-blog/${blog.slug}`} className="max-lg:w-full">
                <figure className="h-96 w-full overflow-hidden lg:h-[190px] lg:w-[464px] lg:flex-1">
                  <Image src={blog.thumbnail} width={464} height={190} className="h-full w-full object-cover transition-all duration-500 group-hover:rotate-3 group-hover:scale-125" alt="Blog Images" />
                </figure>
              </Link>
            </RevealWrapper>
          ))}
