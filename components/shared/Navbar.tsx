  ...
  return (
    <>
      <header ref={headerRef} className="fixed z-[9999] w-full transition-transform duration-300">
        <div className={`pointer-events-none fixed top-0 z-[21] h-[155px] w-full transition duration-300 ease-linear will-change-transform ${isVisible ? "translate-y-0" : "-translate-y-full"}`}>
          <div className="pointer-events-none absolute inset-0" style={{ backdropFilter: "blur(0px)", WebkitBackdropFilter: "blur(0px)" }}></div>
          <div
        ...
        <nav className={`fixed z-[1000] w-full px-5 pt-1 transition duration-300 ease-linear will-change-transform sm:px-8 sm:pt-5 ${isVisible ? "translate-y-0" : "-translate-y-full"}`}>
          <div className="flex justify-between">
            <Link href="/" className="relative z-10">
              <h6>LOGO</h6>
              {/* <Image className="inline-block max-h-[68px] w-auto dark:hidden" src={logo} alt="logo" width={136} height={68} priority />
              <Image className="hidden max-h-[68px] w-auto dark:inline-block" src={logoDark} alt="logo" width={136} height={68} priority /> */}
            </Link>
      ...
      <nav
        ...
        <button ref={closeBtnRef} onClick={closeMenu} className="menu-close sticky left-[89%] top-9 h-[40px] w-[40px] cursor-pointer text-white sm:left-[90%] md:left-[93%] lg:left-[94.7%] xl:left-[96.5%] 2xl:left-[97%]" aria-label="Close Menu">
      ...
      <div ref={menuOverflowRef} className="menu-overflow pointer-events-none fixed inset-0 z-[9999] bg-[rgba(10,10,10,0.95)] backdrop-blur-[25px]"></div>
    </>
  );
}
